...
const processStepsStructure: Omit<ProcessStep, 'title' | 'description'>[] = [
  {
    icon: SearchIcon,
    titleKey: "step1Title",
            ...
            <Card key={index} className="bg-card border-border rounded-xl card-hover p-6 flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-semibold text-foreground font-heading">
                  {dictionary[step.titleKey as keyof typeof dictionary] as string}
                </h3>
                <p className="text-foreground/80 text-sm leading-relaxed">
                  {dictionary[step.descriptionKey as keyof typeof dictionary] as string}
                </p>
              </div>
            </Card>
          ))}
