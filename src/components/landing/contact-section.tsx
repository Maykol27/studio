...
import { saveContactSubmission, type ContactFormData as ServerContactFormData } from '@/app/actions/saveContactSubmission';
...
const texts = {
  ...
  // toastErrorDescriptionGeneral: "No se pudo enviar el mensaje. Por favor, inténtalo de nuevo.", // Se usará el mensaje del server action
  toastErrorDescriptionUnexpected: "Ocurrió un error inesperado. Por favor, inténtalo más tarde."
};

// El schema del frontend puede ser un poco más laxo si la validación final está en el server action
// o ser idéntico para consistencia. Lo mantendremos idéntico por ahora.
...
export function ContactSection() {
  ...
  const onSubmit: SubmitHandler<ContactFormData> = async (data) => {
    ...
    try {
      ...
      const response = await saveContactSubmission(data as ServerContactFormData);

      if (response.success) {
        toast({
          title: texts.toastSuccessTitle,
          description: response.message || texts.toastSuccessDescription,
        });
        reset();
      } else {
        toast({
          title: texts.toastErrorTitle,
          description: response.message || texts.toastErrorDescriptionUnexpected,
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <section id="contact" className="py-10 sm:py-12 md:py-16 bg-background">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <Card className="max-w-2xl mx-auto bg-card border-border rounded-xl shadow-md card-hover">
